...
```
$ cd ~/workspace/paasta-4.0/stemcells
$ bosh -e {director_name} upload-stemcell bosh-aws-xen-hvm-ubuntu-xenial-go_agent-97.28.tgz (aws)
$ bosh -e {director_name} upload-stemcell bosh-openstack-xen-hvm-ubuntu-xenial-go_agent-97.28.tgz (openstack)
$ bosh -e {director_name} upload-stemcell bosh-vsphere-esxi-ubuntu-xenial-go_agent-97.28.tgz (vsphere)
$ bosh -e {director_name} upload-stemcell bosh-azure-hyperv-ubuntu-xenial-go_agent-97.28.tgz.tgz (azure)
$ bosh -e {director_name} upload-stemcell bosh-google-kvm-ubuntu-xenial-go_agent-97.28.tgz (google)
...
다음은 openstack기준 cloud-config설정 예제이다. paasta-4.0 deployment를 다운로드 받으면 cloud-config 디렉토리 이하에 iaas별 cloud-config 예제를 볼 수 있다
